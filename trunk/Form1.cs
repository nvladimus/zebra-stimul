...
namespace Stimulus
{
    public partial class Form1 : Form
    {
            ...
            if (!bStopped)
            {
                ...
                if (readDataL > 0)
                {
                    ...
                    if (started)
                    {
                        ...
                        if (!boolEmulation)
                        {
                            updateDrift(triggerCount, nPlanes, planeOffset, boolSyncToCam);
                        }
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            boolSyncToCam = checkBoxSynchToCam.Checked;
            triggerThresh = Convert.ToDouble(numericUpDownSyncTriggerTresh.Value);
            nPlanes = Convert.ToInt16(numericUpDownNImagingPlanes.Value);
            planeOffset = Convert.ToInt16(numericUpDownPlaneOffset.Value);
   ...
   */
        ...
        private void updateDrift(long triggerCount1, short nPlanes1, short planeOffset1, bool boolSyncToCam1)
        {
            if (!checkBoxTimeInStacks.Checked)
            {
